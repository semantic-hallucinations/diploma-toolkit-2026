...
1. Mermaid/PlantUML/Structurizr использовать как входные языки, но не как
   финальные renderers.
2. Все входы приводить к normalized diagram model.
3. Для строгих ГОСТ-блок-схем сделать отдельный draw.io renderer с
   собственными шаблонами УГО и проверками.
4. Для software-диаграмм использовать draw.io renderer с единым
   черно-белым стилем, но не называть их `ГОСТ 19.701`.
5. Все результаты экспортировать в PNG и пропускать через sanity-check:
   стиль, размер, шрифт, толщина линий, пересечения, текст за границами.
